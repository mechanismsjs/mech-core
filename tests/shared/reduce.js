describe("reduce mechanism - reduce", function () {
   ...
   //    var mech = m.reduce(m.emitFromArr([1,2,3,4]));
   ...
   //    var mech2 = m.reduce(m.emitFromArr([8,6,7]));
   ...
   //    var mech = m.reduce(m.emitFromRange(1,2,.5));
   ...
   //    var mech = m.reduce(m.emitFromRange(1,2000,2),3);
   //    expect(mech.go).to.have.length(3);
   //    expect(mech.go[0]).to.equal(1);
   ...
   //    var mech = m.reduce(m.emitFromRange(1,Infinity,45959532));
   ...
   //    var mech = m.reduce(m.add(4,m.emitFromArr([1,2,3,4])));
   //    expect(mech.go).to.have.length(4);
   ...
   //    var mech3 = m.reduce(m.addS(m.emitFromArr(["a","b","c"]), "e"));
   ...
   // it ("should reduce a literal", function(){
   //    var mech = m.reduce("hello",5);
   ...
   //    var mech = m.reduce(m.emitFromRange(1,Infinity,45959532),m.num(5));
   //    expect(mech.go).to.have.length(5);
   // });
   
   // it ("should not hang on an invalid mechanism result", function(){
   //    var mech = m.reduce(m.emitFromRange(1,Infinity,45959532),m.num(undefined));
